...
- **Port Verification**: WHY? Prevent `EADDRINUSE` crashes during server startup. Dynamically use OS-native tools. *(Example: `Get-NetTCPConnection -LocalPort <port>` or `netstat -ano | findstr <port>` on Win, `lsof -i :<port> -t` on Unix)*.
